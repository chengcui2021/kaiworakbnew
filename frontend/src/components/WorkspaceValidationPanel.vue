...
import type { WorkspaceValidationResult } from '../types/domain'
...
const result = ref<WorkspaceValidationResult | null>(null)
...
async function runValidation() {
  ...
  error.value = null
  try {
    result.value = await validateWorkspace(props.workspaceId)
  } catch (err) {
...
watch(() => props.workspaceId, runValidation)
</script>

<template>
  <Card data-test="validation-panel">
    <CardHeader class="flex-row items-center justify-between space-y-0">
      <CardTitle>Workspace Validation</CardTitle>
